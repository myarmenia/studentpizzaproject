...
const AuthService = {
  signUp: async(email,password,firstname,lastname,username)=>{
    const [findUsername, findEmail] = await Promise.all([User.findOne({username}), User.findOne({email})])
  ...
  signIn: async(email,password)=>{
    ...
    if(user){
      if(bcrypt.compareSync(password,user.password)){
        ...
        return {message: "You Have Successfully Logged In", access_token,refresh_token}
  ...
  refresh: async(refresh_token)=>{
   if(refresh_token){
    ...
    if(token){
      const newTokens = jwt.verify(refresh_token,process.env.REFRESH_TOKEN, async (err,data)=>{
          if(err) console.error(err)

          const deleteToken = await Token.findOneAndDelete({token: refresh_token})
          ...
          return {message: "Token is Refreshed", access_token:newAccessToken, refresh_token: newRefreshToken}
      })
  ...
  signOut: async(refresh_token)=>{
    if(refresh_token){
      const deleteRefreshToken = await Token.findOneAndDelete({token: refresh_token})
